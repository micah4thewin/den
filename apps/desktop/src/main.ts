...
function renderReport(report: Report): void {
  ...
  for (const entry of report.entries) {
    ...
    file.textContent = entry.input.split("/").pop() ?? entry.input;
    file.title = entry.input;
    row.appendChild(file);

    const detail = Object.values(entry.outcome.detail).filter(Boolean).join(" · ");
...
async function intakeFolder(path: string): Promise<void> {
  ...
  try {
    ...
    status.classList.remove("breathe");
  }
}
