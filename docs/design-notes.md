# Play (den) and the shared design system
...
all four programs; this note is Play's adaptation, in the same spirit as
lanflix's — only the places where a launcher differs.

Play is a fourth program on the same interface: greyscale by rule, three
materials, one icon geometry, a container holding one emblem.

## The mark

**Play** is a CRT with a play triangle in it — a container holding one emblem,
...
## Where Play differs from the document

- **No syntax exception.** The `--syn-*` seeds are carried in `tokens.css`
  because the seed list is shared, but Play displays no code and no diffs, so
  nothing reads them. Like hearth, Play has no case for the exception.

- **No handwriting faces.** Lockbox bundles Caveat, Architects Daughter, and
  Patrick Hand for its whiteboard. Play has no whiteboard, so it bundles
  ...
  that can only fail. Play ships Gilroy and Avenir Next LT Pro and asks for
  nothing else.

- **No lock screen.** Play keeps a library of files that are already on the
...
each one a word on a raised tag, each one legible printed in grey. `quarantined` is not
